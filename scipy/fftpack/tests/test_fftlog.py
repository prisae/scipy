...
from scipy.fftpack.helper import rfftlogargs

from numpy import pi


class TestRFFTlogargs(TestCase):

    def test_definition(self):
        # Default values in rfftlogargs:
        # - dlogr : 0.01
        # - logrc : 0.0
        # - mu : 0.5
        # - q : 0
        ...
        out = rfftlogargs(5, mu=-0.5, kr=pi)
        assert_array_almost_equal(inppts, out[0])
        assert_array_almost_equal(outpts, out[1])
        assert_equal(kr, out[2])
        assert_equal(rk, out[3])
        ...
        out = rfftlogargs(n=5, logrc=1, kropt=1)
        assert_array_almost_equal(inppts, out[0])
        assert_array_almost_equal(outpts, out[1])
        assert_equal(kr, out[2])
        assert_equal(rk, out[3])
